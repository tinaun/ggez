...
impl MainState {
    fn new(ctx: &mut Context) -> GameResult<MainState> {
        ...
        let s = MainState { image, spritebatch: batch };
        Ok(s)
    }
}
...
impl MainState {
    ...
    fn draw_normally(&mut self, ctx: &mut Context) -> GameResult {
    ...
    fn draw_with_spritebatch(&mut self, ctx: &mut Context) -> GameResult {
        let time = (timer::duration_to_f64(timer::get_time_since_start(ctx)) * 1000.0) as u32;
        let cycle = 10_000;
        for x in 0..150 {
            for y in 0..150 {
...
impl event::EventHandler for MainState {
    ...
    fn draw(&mut self, ctx: &mut Context) -> GameResult {
        ...
        // self.draw_with_spritebatch(ctx)?;
        self.draw_normally(ctx)?;
